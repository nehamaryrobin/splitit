...
const load = () => { try { return JSON.parse(sessionStorage.getItem(GUEST_KEY)) || null; } catch { return null; } };
const save = (d) => sessionStorage.setItem(GUEST_KEY, JSON.stringify(d));
...
function InlineTripName({ value, onChange }) {
  ...
  if (editing) {
    return (
      <div className="flex items-center justify-center gap-2">
        ...
        />
        <button onClick={commit}  className="text-green-500 hover:text-green-600 transition-colors"><Check className="h-5 w-5" /></button>
        <button onClick={cancel}  className="text-slate-400 hover:text-slate-600 transition-colors"><X    className="h-5 w-5" /></button>
      </div>
    );
  }
...
function ExpenseDialog({ open, onClose, onSave, participants, categories, catEnabled, currency, initial }) {
  ...
  const handleSave = () => {
    if (!form.name.trim()) return toast({ title: 'Expense name required', variant: 'destructive' });
      ...
      <DialogContent className="max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4 px-6 py-2">
          <div className="space-y-1.5">
            ...
            <Input placeholder="e.g. Dinner, Cab, Hotel…" value={form.name}
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-1.5">
              <Label>Paid by</Label>
              <Select value={form.paidBy} onValueChange={set('paidBy')}>
          ...
          {catEnabled && categories.length > 0 && (
            <div className="space-y-1.5">
              ...
              <Select value={form.category} onValueChange={set('category')}>
                ...
                <SelectContent>
                  {['General', ...categories].map(c => <SelectItem key={c} value={c}>{c}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
          )}
          <div className="space-y-1.5">
            ...
            <SplitToggle participants={participants} selected={form.splitBetween}
              onChange={v => setForm(f => ({ ...f, splitBetween: v }))} />
          </div>
        </div>
        <DialogFooter>
...
export default function GuestPage() {
  ...
  useEffect(() => { save(trip); }, [trip]);

  const upd = (patch) => setTrip(t => ({ ...t, ...patch }));
  ...
  const addParticipant = () => {
    ...
    upd({ participants: [...trip.participants, name] });
    setPInput('');
  };
  ...
  const addCategory = () => {
    ...
    upd({ categories: [...trip.categories, name] });
    setCatInput('');
  };
  ...
  const saveExpense = (data) => {
    if (dialog.editing !== null) {
      upd({ expenses: trip.expenses.map((e, i) => i === dialog.editing ? { ...e, ...data } : e) });
      toast({ title: 'Expense updated' });
    } else {
      upd({ expenses: [...trip.expenses, { ...data, id: crypto.randomUUID() }] });
      toast({ title: 'Expense added' });
    }
    setComputed(false);
  };

  const deleteExpense = (idx) => { upd({ expenses: trip.expenses.filter((_, i) => i !== idx) }); setComputed(false); };

  // ── Compute ───────────────────────────────────────────────────
  const compute = () => {
    if (!trip.expenses.length) return toast({ title: 'Add at least one expense first', variant: 'destructive' });
  ...
  const personData = trip.participants.map((p, i) => {
    const val = trip.expenses.reduce((s, e) => s + (e.splitBetween.includes(p) ? e.amount / e.splitBetween.length : 0), 0);
  ...
  const catData = (() => {
    ...
    const map = {};
    trip.expenses.forEach(e => { const c = (e.category && e.category !== 'General') ? e.category : 'General'; map[c] = (map[c] || 0) + e.amount; });
    return Object.entries(map).map(([name, value], i) => ({ name, value: Math.round(value * 100) / 100, color: CHART_PALETTE[i % CHART_PALETTE.length] }));
  })();
    ...
    <AuthBackground>
      <div className="relative z-10 flex min-h-screen flex-col">
        ...
        <header className="flex items-center justify-between px-5 py-4 sm:px-8">
          <Link to="/" className="flex items-center gap-2 text-[15px] font-bold text-slate-800">
              ...
              <Card>
                ...
                <CardContent className="space-y-3">
                  {trip.participants.length > 0 && (
                    <div className="flex flex-wrap gap-2">
                      {trip.participants.map(p => (
                        <Chip key={p} label={p} onRemove={() => upd({ participants: trip.participants.filter(x => x !== p) })} />
                      ))}
                    </div>
                  )}
                  <div className="flex gap-2">
                    ...
                    <Button variant="outline" onClick={addParticipant} disabled={trip.settled}>Add</Button>
                  </div>
                </CardContent>
              </Card>

              {/* Category toggle */}
              <label className={cn("flex cursor-pointer items-center gap-3 rounded-xl border border-border bg-card px-4 py-3 shadow-sm", trip.settled && "opacity-50 pointer-events-none")}>
                <input type="checkbox" className="h-4 w-4 accent-primary"
                  checked={trip.catEnabled} disabled={trip.settled} onChange={e => upd({ catEnabled: e.target.checked })} />
                <span className="text-sm text-foreground font-medium">Enable expense categories</span>
                <span className="ml-auto text-xs text-muted-foreground">Optional</span>
              </label>

              {/* Categories */}
              {trip.catEnabled && (
                <Card className="animate-in slide-in-from-top-1 duration-200">
                  <CardHeader className="pb-3">
                    ...
                    {trip.categories.length > 0 && (
                      <div className="flex flex-wrap gap-2">
                        {trip.categories.map((c, i) => (
                          <CatChip key={c} label={c} idx={i} onRemove={() => upd({ categories: trip.categories.filter(x => x !== c) })} />
                        ))}
                      </div>
                    )}
                    <div className="flex gap-2">
                      <Input placeholder="e.g. Food, Travel, Hotel…" value={catInput} className="flex-1"
              ...
              <Card>
                <CardHeader className="pb-3">
                  <div className="flex items-center justify-between">
                    ...
                    <Select value={trip.currency} onValueChange={v => upd({ currency: v })}>
                ...
                <CardContent>
                  <div className="max-h-[calc(100vh-440px)] min-h-[64px] space-y-2 overflow-y-auto pr-1">
                    {trip.expenses.length === 0
                      ...
                      : trip.expenses.map((e, i) => {
                          ...
                          return (
                            <div key={e.id || i} className="rounded-lg border border-border bg-secondary/40 px-3 py-2.5">
                  ...
                  <Button
                    variant="secondary" className="mt-3 gap-2"
                    disabled={trip.settled}
                    onClick={() => {
              ...
              <div>
                {trip.settled ? (
                  ...
                  <p className="text-sm text-muted-foreground">
                    Done splitting? Mark this trip as settled to lock it.
                  </p>
                )}
              </div>
              <button
